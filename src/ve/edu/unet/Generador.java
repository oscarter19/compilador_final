...
public class Generador {
	...
	if(tablaSimbolos!=null) {
		if (nodo instanceof NodoIf) {
			...
			generarFor(nodo);
		} else if (nodo instanceof NodoVariableBooleana){
			...
			generarVector(nodo);
		} else if (nodo instanceof NodoOperacionVector){
			generarOperacionVector(nodo);
		}
		else{
	...
	private static void generarOperacion(NodoBase nodo){
		...
		switch(n.getOperacion()){
			...
			case not:
							...
							break;
			case modulo:    UtGen.emitirRO("MOD", UtGen.AC, UtGen.AC1, UtGen.AC, "op: %");
							break;
			default:
	...
	private static void generarFor(NodoBase nodo){
		...
		UtGen.emitirRM_Abs("LDA", UtGen.PC, localidadCondicion, "for: regresar a la condición");
